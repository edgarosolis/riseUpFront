...
const AssessmentPDF = ({ data, sections, userName, is360 }) => {
    ...
    <Document>
        ...
        <Page style={{ paddingBottom: 110 }}>
            <BgLogo />
            <BannerPDF title={"Kingdom Calling Assessment"} subtitle={is360 ? "360-DEGREE REPORT" : "SELF-ASSESSMENT REPORT"} userName={userName} completedAt={data.submission?.completedAt || data.submission?.updatedAt} reviewerCount={is360 ? reviewerCount : 0}/>
        ...
        <Page style={{ paddingBottom: 110 }}>
            ...
            {is360 && (
                <View style={{ backgroundColor: '#FFF8E1', paddingHorizontal: 50, paddingBottom: 25, paddingTop: 15, marginTop: 15 }}>
            ...
            sections.map((s,i)=>{
                ...
                <Fragment key={i}>
                    ...
                    {(allQs.length > 0 || reviewerResult) && (
                        <Page style={{ paddingBottom: 110, paddingTop: 0 }}>
                            <BgLogo />
                            {/* Fixed header for overflow pages */}
                            <View style={[stylesPDF.bannerMiniContainer, { backgroundColor: s.color }]} fixed render={({ pageNumber, subPageNumber }) => subPageNumber > 0 ? (
